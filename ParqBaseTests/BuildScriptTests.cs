namespace ParqBaseTests
{
    ...
    /// <summary>
    /// End-to-end regression test for the demo script <c>ParqScripts/build.sql</c>, which exercises
    ...
    public class BuildScriptTests
    {
        ...
        {
            var scriptPath = Path.Combine(this.repoRoot, "ParqScripts", "build.sql");
        ...
        private int Count(ParqBase db, string table)
        {
        ...
        private static string FindRepoRoot()
        {
            ...
            while (dir != null && !File.Exists(Path.Combine(dir.FullName, "ParqBase.sln")))
            {
                dir = dir.Parent;
            }

            return dir?.FullName ?? throw new InvalidOperationException("Could not locate repository root (ParqBase.sln).");
        }
    }
}
