namespace TidalSqlLib
{
    ...
    public partial class TidalSql
    {
        ...
        // Retained for dependency-injection resolution; TidalSql keeps its own session state.
        public TidalSql(IServiceProvider serviceProvider)
        {
        }

        /// <summary>
        /// Root folder for all persisted data (databases and the server security catalog).
        /// Defaults to <c>%ProgramData%\TidalSql</c> (a machine-wide location outside any user's
        /// source tree, so it can be ACL-locked to a dedicated service account the same way SQL
        /// Server protects its data files), and can be overridden with the TIDALSQL_DATA_ROOT env var.
        /// </summary>
        ...
        {
            // A TidalSql instance represents a single logical session; serialize calls so a
            // shared instance stays consistent. Concurrency comes from using separate instances.
            lock (this.gate)
            {
                var visitor = new TidalSqlStatementVisitor(this.session, cancellationToken);
        ...
        public TableInfo GetTableInfo(string database, string table, string schema = "dbo")
        {
            ...
            {
                rowCount = TidalSqlStatementVisitor.CountRows(source.Files);
                ...
                if (source.IsMultiPart)
                {
                    ...
                    meta = null;
                    columnSource = TidalSqlStatementVisitor.ReadSchemaColumns(source.PrimaryFile);
                ...
                else
                {
                    ...
                    meta = TidalSqlStatementVisitor.LoadTableMeta(source.PrimaryFile);
                    columnSource = meta?.Columns
                        ?? TidalSqlStatementVisitor.EnsureTableMeta(source.PrimaryFile).Columns;
                    ...
                    var metaPath = TidalSqlStatementVisitor.MetaSidecarPath(source.PrimaryFile);
            ...
            for (var i = 0; i < columnSource.Count; i++)
            {
                ...
                var (dataType, maxLength, precision, scale) =
                    TidalSqlStatementVisitor.DescribeColumnType(column);
        ...
        public string GetProcedureDefinition(string database, string procedure, string schema = "dbo")
        {
            ...
            var fileName = TidalSqlStatementVisitor.ProcedureFileName(schema, procedure);
            ...
            return TidalSqlStatementVisitor.BuildProcedureDefinition(filePath);
        }

        /// <summary>
        /// Determines the persistent data root. Prefers the TIDALSQL_DATA_ROOT environment variable;
        /// otherwise uses the machine-wide <c>%ProgramData%\TidalSql</c> folder. Keeping data in
        /// ProgramData (rather than inside a user's source tree) lets an administrator restrict the
        /// directory's NTFS ACLs to a dedicated TidalSql service account, so interactive users cannot
        ...
        {
            var configured = Environment.GetEnvironmentVariable("TIDALSQL_DATA_ROOT");
            ...
            var root = Path.Combine(programData, "TidalSql");
    ...
    /// <summary>
    /// Outcome of <see cref="TidalSql.InitializeServer(string)"/>. When <see cref="Created"/> is
    ...
    /// <summary>
    /// The level of database access granted by <see cref="TidalSql.GrantDatabaseAccess"/>: read-only
    ...
    /// <summary>
    /// Catalog metadata describing a single table, returned by <see cref="TidalSql.GetTableInfo"/>.
    /// </summary>
