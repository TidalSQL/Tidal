namespace TidalSqlConsole
{
    ...
    internal class TidalSqlHostedService : BackgroundService
    {
        private const int MaxLoginAttempts = 3;

        private readonly TidalSql db;
        private readonly IHostApplicationLifetime lifetime;

        public TidalSqlHostedService(TidalSql db, IHostApplicationLifetime lifetime)
        {
            this.db = db;
        ...
        {
            Console.WriteLine("Initializing TidalSql...");

            // Seed the default administrator on first run and surface its credentials once.
            var defaultPassword = Environment.GetEnvironmentVariable("TIDALSQL_ADMIN_PASSWORD");
            ...
            return Task.Run(() =>
            {
                try
                {
                    ...
                    Console.WriteLine();
                    Console.WriteLine($"TidalSql is running. Signed in as '{this.db.CurrentLogin}'.");
        ...
        {
            IReadOnlyList<TidalSqlLib.QueryResult> results;
            try
            {
